CREATE
OR REPLACE PACKAGE BODY course_pkg IS
...
PROCEDURE get_all_courses (allcourse OUT SYS_REFCURSOR) IS BEGIN OPEN allcourse FOR
SELECT
...
) IS BEGIN OPEN count_and_title_of_course FOR
SELECT
    c.title,
    COUNT(t.id)
FROM
